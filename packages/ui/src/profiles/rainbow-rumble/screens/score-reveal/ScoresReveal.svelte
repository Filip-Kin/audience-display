...
<div class="rr fixed z-10 grid grid-cols-[.36fr_.28fr_.36fr] w-full h-full p-8 gap-8" class:flex-row-reverse={$settings.invert}>
	{#if results && ready}
		...
		<div>
			...
			<div
				class="h-60 flex items-center justify-center rounded-[var(--rr-r)] bg-[oklch(0_0_0/0.35)] p-6"
		...
		<!-- Cell 3: right column, FUN Network livestream partner -->
		<div>
...
<style>
	/* ScoreBreakdown is shared and can't be edited; give it the RR white-card
	   look (rounded corners, 32px rows) from outside. */
	.rr-breakdown > :global(div) {

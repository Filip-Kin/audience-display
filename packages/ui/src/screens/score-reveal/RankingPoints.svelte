<script lang="ts">
  ...
  $: if ($state.results) {
    ...
    if (blue.coralBonusRP)
      rankingPoints.push({ src: "/coral-white.png", alt: "coral" });
    if (blue.bargeBonusRP)
      rankingPoints.push({ src: "/barge.svg", alt: "barge" });
...
{#if $state.results && ready}
  <div class="flex flex-col gap-3 justify-center" out:fade={{ duration: 100 }}>
    <span class="text-2xl text-center font-bold" in:fade={{ duration: 100 }}
      >Ranking Points</span
    >

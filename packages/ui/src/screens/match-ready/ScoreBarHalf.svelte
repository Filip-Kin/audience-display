	...
	<div class="flex flex-row justify-start">
		<div class="bg-{alliance}-600 flex flex-row p{invert ? 'l' : 'r'}-16 -m{invert ? 'l' : 'r'}-16 rounded-{invert ? 'r' : 'l'}-xl relative rainbow-shadow">
			<div class="flex flex-row z-10 relative" style={`max-width: ${$wingSpring / 2}vw; opacity: ${$opacityTween}`} class:flex-row-reverse={invert}>
				<div class="flex flex-col justify-center px-3 w-32 text-6xl font-bold text-center">
					{score.score}
				</div>
				...
				<div class="flex flex-col px-4 text-3xl justify-center text-center text-nowrap min-w-24 gap-2 my-2">
					<div class="flex justify-between bg-white text-black rounded-full py-1.5 px-4 items-center gap-3" class:flex-row-reverse={invert}>
						<span class="grow text-center mb-1">{score.algaeCount}</span>
						<img src="/algea.png" alt="algea" class="size-8" />
					</div>
					<div
						class="flex justify-between bg-white text-black rounded-full py-1.5 px-4 items-center gap-2"
						class:bg-yellow-200={score.coralBonusRP}
						class:flex-row-reverse={invert}
					>
						<span class="grow text-center mb-1">{score.coralBonusProgress} / {score.coralBonusThreshold}</span>
						<img src="/coral.png" alt="coral" class="size-8" />
					</div>
				</div>

				<div class="flex flex-col justify-center px-3 w-32 gap-1">
					{#each $state.match.teams[alliance] as team, index}
						<span class="text-3xl text-center text-black rounded {team.card ? 'bg-yellow-200' : 'bg-white'}">{team.number}</span>
					{/each}
				</div>
			</div>
		</div>
